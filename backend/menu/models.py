...
class MenuItem(models.Model):
    ...
    # Ikki xil tartib raqami
    global_order = models.PositiveIntegerField(
        default=0, 
        help_text="Barcha taomlar orasidagi umumiy tartib (Hammasi kategoriyasi uchun)"
    )
    category_order = models.PositiveIntegerField(
        default=0, 
        help_text="Faqat o'z kategoriyasi ichidagi tartib"
    )
    ...
    class Meta:
        ...
        # Ikki xil tartiblash imkoniyati
...
class SiteSettings(models.Model):
    ...
    site_name = models.CharField(max_length=200, default="Pizza Centr Garden")
    site_name_uz = models.CharField(max_length=200, default="Pizza Centr Garden")
    ...
    email = models.EmailField(default="info@pizzacentr.uz")
...
class RestaurantInfo(models.Model):
    ...
    restaurant_name = models.CharField(max_length=200, default="Pizza Centr Garden")
    restaurant_name_uz = models.CharField(max_length=200, default="Pizza Centr Garden")
    restaurant_name_ru = models.CharField(max_length=200, default="Пицца Центр Гарден")
    ...
    # Hero Section
    hero_title = models.CharField(max_length=200, default="Pizza Centr Garden")
    hero_subtitle = models.CharField(
        max_length=500, 
        default="O'zbekistondagi eng mazali pizza va grill taomlarini tatib ko'ring"
    )
    ...
    hero_subtitle_ru = models.CharField(
        max_length=500, 
        default="Попробуйте самую вкусную пиццу и гриль-блюда в Узбекистане"
    )
...
class FeaturedDish(models.Model):
    """Manually selected dishes to be featured on the home page"""
    menu_item = models.ForeignKey(MenuItem, on_delete=models.CASCADE, related_name='featured_as')
    ...
    def __str__(self):
        return f"{self.menu_item.name} (Order: {self.order})"


# ============================================
# Signals: Auto-delete images from filesystem
# ============================================

@receiver(post_delete, sender=MenuItem)
def auto_delete_menu_item_image(sender, instance, **kwargs):
    """MenuItem o'chirilganda rasmni ham o'chirish"""
    if instance.image:
...
@receiver(pre_save, sender=MenuItem)
def auto_delete_old_menu_item_image(sender, instance, **kwargs):
    """MenuItem rasmi o'zgarganda eski rasmni o'chirish"""
    if not instance.pk:
        return
    try:
...
def auto_delete_category_image(sender, instance, **kwargs):
    """Category o'chirilganda rasmni ham o'chirish"""
    if instance.image:
        if os.path.isfile(instance.image.path):
            os.remove(instance.image.path)
...
def auto_delete_old_category_image(sender, instance, **kwargs):
    """Category rasmi o'zgarganda eski rasmni o'chirish"""
    if not instance.pk:
        return
    try:

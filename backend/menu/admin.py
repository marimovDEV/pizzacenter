...
from .models import Category, MenuItem, Promotion, Order, OrderItem, SiteSettings, TextContent, RestaurantInfo, Cart, CartItem, FeaturedDish
from .forms import PromotionForm, MenuItemForm, SiteSettingsForm


# Custom admin site configuration
admin.site.site_header = "🍽️ Pizza Centr Garden Admin"
admin.site.site_title = "Pizza Centr Garden Admin"
...
class PromotionAdmin(ModelAdmin):
    ...
    @display(description="Linked Product")
    def linked_product_display(self, obj):
        if obj.linked_product:
            return format_html(
                '<strong>{}</strong><br><small style="color: #666;">{}</small>',
                obj.linked_product.name,
                obj.linked_product.category.name if obj.linked_product.category else 'No Category'
            )
        return "No linked product"
    
    def get_queryset(self, request):
        return super().get_queryset(request).select_related('linked_product', 'promotion_category')
...
@admin.register(FeaturedDish)
class FeaturedDishAdmin(ModelAdmin):
    list_display = ['menu_item', 'order', 'is_active', 'created_at']
    list_filter = ['is_active', 'created_at']
    list_editable = ['order', 'is_active']
    search_fields = ['menu_item__name', 'menu_item__name_uz', 'menu_item__name_ru']
